...
- Enhanced to capture detailed spending transactions and portfolio data
"""
...
def setup_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS portfolio_log (
            ...
            nasdaq_value DECIMAL(12,2),
            btcc_value DECIMAL(12,2),
            zsp_value DECIMAL(12,2),
            trade_cash DECIMAL(12,2),
...
def migrate_portfolio_data():
    ...
    try:
        # Read the Trades sheet from Personal Log.xlsx
        df = pd.read_excel('Personal Log.xlsx', sheet_name='Trades')
        
        conn = sqlite3.connect('finance_tracker.db')
        cursor = conn.cursor()
        
        # Clear existing data
        cursor.execute('DELETE FROM portfolio_log')
        cursor.execute('DELETE FROM etf_holdings')
        
        migrated_portfolio = 0
        migrated_etfs = 0
        
        # Migrate daily portfolio data (A2:I onwards)
        for _, row in df.iterrows():
            # Skip header row and empty dates
            if pd.isna(row.iloc[0]) or row.iloc[0] == 'Date':
                continue
                
            try:
                # Column A = Date
                date_obj = pd.to_datetime(row.iloc[0]).date()
                
                # Column B = Total Portfolio Value (Amount)
                total_value = None
                if not pd.isna(row.iloc[1]):
                    total_value = float(str(row.iloc[1]).replace(',', ''))
                
                # Column C = NASDAQ value
                nasdaq_value = None
                if not pd.isna(row.iloc[2]):
                    nasdaq_value = float(str(row.iloc[2]).replace(',', ''))
                
                # Column D = BTCC value
                btcc_value = None
                if not pd.isna(row.iloc[3]):
                    btcc_value = float(str(row.iloc[3]).replace(',', ''))
                
                # Column E = ZSP value
                zsp_value = None
                if not pd.isna(row.iloc[4]):
                    zsp_value = float(str(row.iloc[4]).replace(',', ''))
                
                # Column F = Trade Cash
                trade_cash = None
                if len(row) > 5 and not pd.isna(row.iloc[5]):
                    trade_cash = float(str(row.iloc[5]).replace(',', ''))
                
                # Column G = Difference
                difference = None
                if len(row) > 6 and not pd.isna(row.iloc[6]):
                    try:
                        difference = float(str(row.iloc[6]).replace(',', ''))
                    except:
                        pass  # Some cells have text like "Mid one"
                
                # Column I = Days (skipping H which is TOTAL)
                days = None
                ...
                cursor.execute('''
                    ...
                    (date, total_portfolio_value, nasdaq_value, btcc_value, zsp_value, trade_cash, difference, days)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (date_obj, total_value, nasdaq_value, btcc_value, zsp_value, trade_cash, difference, days))
                
                migrated_portfolio += 1
                
            except Exception as e:
                print(f"   ⚠️  Error processing portfolio row: {e}")
                continue
        
        # Migrate ETF holdings data (columns O and P)
        # Look for ETF symbols in column O (index 14) and purchase values in column P (index 15)
        for _, row in df.iterrows():
            if len(row) > 15:  # Make sure we have columns O and P
                etf_symbol = row.iloc[14]  # Column O
                purchase_value = row.iloc[15]  # Column P
                
                # Skip if either is empty
                if pd.isna(etf_symbol) or pd.isna(purchase_value) or str(etf_symbol).strip() == '':
                    continue
                
                try:
                    etf_symbol = str(etf_symbol).strip()
                    purchase_value = float(str(purchase_value).replace(',', '').replace('$', ''))
                    
                    cursor.execute('''
                        INSERT INTO etf_holdings (etf_symbol, purchase_value)
                        VALUES (?, ?)
                    ''', (etf_symbol, purchase_value))
        ...
        print(f"   ✅ Migrated {migrated_portfolio} portfolio records")
        print(f"   ✅ Migrated {migrated_etfs} ETF holding records")
...
def verify_migration():
    ...
    cursor.execute('SELECT COUNT(*) FROM portfolio_log')
    ...
    cursor.execute('SELECT date, total_portfolio_value, nasdaq_value, btcc_value, zsp_value FROM portfolio_log ORDER BY date DESC LIMIT 5')
    ...
    print(f"   📈 Portfolio entries: {portfolio_count} records")
    print(f"   🏦 ETF holdings: {etf_count} records")
    ...
    if recent_portfolio:
        ...
        for date, total, nasdaq, btcc, zsp in recent_portfolio:
            # Handle None values safely
            total_str = f"${total:.2f}" if total is not None else "N/A"
            nasdaq_str = f"${nasdaq:.2f}" if nasdaq is not None else "N/A"
            btcc_str = f"${btcc:.2f}" if btcc is not None else "N/A"
            zsp_str = f"${zsp:.2f}" if zsp is not None else "N/A"
            print(f"     {date}: Total={total_str}, NASDAQ={nasdaq_str}, BTCC={btcc_str}, ZSP={zsp_str}")
    
    if etf_holdings:
        print(f"\n   🏦 ETF holdings:")
        for symbol, value in etf_holdings:
